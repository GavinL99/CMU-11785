...
class UtteranceDataset(Dataset):
    ...
    @classmethod
    def decode_symbols(cls, char_to_num_dict, num_to_char_dict, decode_input):
        """

        :param char_to_num_dict: list of chars
        :param num_to_char_dict:
        :param decode_input: a list of list of char idx
        :return:
        """
        if type(decode_input) == torch.Tensor:
            s = decode_input.detach().tolist()
        output = []

        for s in decode_input:
            assert s[0] == char_to_num_dict["<sos>"]
            temp_output = ""
            for i in range(1, len(s) - 1):
                if s[i] == char_to_num_dict["<eos>"]:
                    break
                if s[i] == char_to_num_dict["<blank>"]:
                    ...
                    temp_output += str(num_to_char_dict[s[i]])
